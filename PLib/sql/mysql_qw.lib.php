...
function mysql_qw() {
  ...
  return $conn!==null? sql_query($query, $conn) : sql_query($query);
}
...
function mysql_make_qw() {
  ...
  foreach ($args as $i=>$v) {
    ...
    $args[$i] = "'". (!get_magic_quotes_gpc() ? mysql_real_escape_string($v) : $v) ."'";
  }
...
function print_rx() {
  ...
  print '<pre>' . call_user_func_array("sprintf", $args) . '</pre>';
}
?>